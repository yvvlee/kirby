...
func TestImportRecordClaimBindsSourceAndTargetScope(t *testing.T) {
	...
	mock.ExpectExec(`(?s)INSERT INTO import_records.*source_snapshot\.id.*source_environment\.project_id = source_project\.id.*target_environment\.project_id = target_project\.id.*ON DUPLICATE KEY UPDATE id = LAST_INSERT_ID\(import_records\.id\)`).
