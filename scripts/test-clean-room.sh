...
export GOPROXY="${KIRBY_GO_PROXY:-https://proxy.golang.org,direct}"
...
server_build_args="--build-arg GOPROXY=$GOPROXY"
# shellcheck disable=SC2086
docker buildx build --builder "$builder_name" --pull --no-cache --load $server_build_args -t "$server_image" -f "$checkout_dir/Dockerfile" "$checkout_dir"

test "$(docker image inspect "$server_image" --format '{{.Config.User}}')" = "65532:65532"
"$checkout_dir/scripts/ci/check-server-image.sh" "$server_image"
